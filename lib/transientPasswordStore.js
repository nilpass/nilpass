...
window.transientPasswordStore = (function(){
  ...
  // Domains using another domain's password
  ...
  function serializableStatusData() {
    return {
      passwords: passwords.entries().map(
        ([domain, {password, expiry}]) => ({domain, password, expiry})),
      crossovers: crossovers.entries().map(
        ([domain, using]) => ({domain, using}))
    };
  }
  ...
  // that persists beyond a page's lifetime, while still being
  // transient within the browser's lifetime. See http://crbug.com/42599
  ...
  function deserializeFromCookie() {
    ...
    for (let {domain, password, expiry} of serializedState.passwords) {
      passwords.set(domain, {password, expiry});
    }
    for (let {domain, using} of serializedState.crossovers) {
      crossovers.set(domain, using);
    }
    processExpiries();
  }
